...
/*   Updated: 2019/01/13 15:43:43 by gperez           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	debug_objs(t_obj *save, t_clip *clips)
{
	...
	while (save != NULL)
	{
		...
		save->m.pt.z);
		ft_printf("\033[38;5;136m");
		clips = save->clips;
		while (clips != NULL)
		{
			ft_printf("Clip : %d\n", clips->id);
			if (clips->obj != NULL)
				ft_printf("Linked to %s:%d\n",
